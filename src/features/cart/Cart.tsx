						...
						Object.entries(items).map(([id, quantity]: any) => (
							<li key={id} id={id}>
								...
								<div className="cart-list__title">
									<h2>
										<strong className={products[id].acessorios ? 'custom-color-tertiary' : ''}>{products[id].tipoConexao}</strong>
										{products[id].nome}<br />
										<small className={products[id].acessorios ? 'custom-color-tertiary' : ''}>{products[id].fixacao} | {products[id].value}</small>
									</h2>
								</div>
